...
<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12" style="margin-top:100px;">
    <div class="card">
        ...
        <div class="col-sm-4">
            <input type="submit" class="btn btn-primary"  value="Search Question" style="margin-left:40px;margin-right:40px;margin-top: 20px;">
        </div>
        </div>
    </div>
    </form>
 ...
 <div class="table-responsive " style="display:block;">
                <table class="table table-bordered">
                    ...
                    <tbody>
                       <?php $i=1;
                       $sql = "SELECT * from subject as d INNER JOIN question as u ON d.subject_id=u.subject_ids where d.subject_id='".$_GET['search']."' order by u.id DESC";
                        $res= mysqli_query($con, $sql);
                        while($row=mysqli_fetch_array($res)) { 
                        if($row) {
                        ?>
           ...
           <?php } else { ?>
            <div class="table-responsive " style="display:block;">
                <table class="table table-bordered">
                    ...
                    <tbody>
                       <?php $i=1;
                       $sql = "SELECT * from subject as d INNER JOIN question as u ON d.subject_id=u.subject_ids order by u.id DESC";
                        $res= mysqli_query($con, $sql);
                        while($row=mysqli_fetch_array($res)) { 
                        if($row) {
                        ?>
